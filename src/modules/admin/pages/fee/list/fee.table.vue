<template>
  ...
  >
    ...
    <slot :slot="'sentai_id'" slot-scope='record' name='sentai_id' :record='record'/>
    <slot :slot="'fee_descriptions'" slot-scope='record' name='fee_descriptions' :record='record'/>
  </a-table>
</template>
...
export default {
  ...
  methods: {
    ...
    onEditing(model) {
      ...
      this.setSelectedFee(this.selected_model)
      this.show_form();
    }
  },
  mounted() {

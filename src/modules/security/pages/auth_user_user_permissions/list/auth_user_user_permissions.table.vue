<template>
  ...
  >
    ...
    <slot :slot="'user_id'" slot-scope='record' name='user_id' :record='record'/>
    <slot :slot="'permission_id'" slot-scope='record' name='permission_id' :record='record'/>
  </a-table>
</template>
...
export default {
  name: "auth_user_user_permissions_table",
  ...
  inject: {
    ...
    setSelectedAuth_user_user_permissions: {
      default: () => {
      }
    }
  },
  ...
  methods: {
    ...
    onEditing(model) {
      ...
      this.setSelectedAuth_user_user_permissions(this.selected_model)
      this.show_form();
    }
  },
  mounted() {

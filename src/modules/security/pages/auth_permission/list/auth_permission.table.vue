<template>
  ...
  >
    ...
    <slot :slot="'name'" slot-scope='record' name='name' :record='record'/>
    <slot :slot="'content_type_id'" slot-scope='record' name='content_type_id' :record='record'/>
    <slot :slot="'codename'" slot-scope='record' name='codename' :record='record'/>
  </a-table>
</template>
...
export default {
  ...
  methods: {
    ...
    onEditing(model) {
      ...
      this.setSelectedAuth_permission(this.selected_model)
      this.show_form();
    }
  },
  mounted() {
